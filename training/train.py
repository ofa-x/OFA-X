...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.checkpoint.upload_checkpoints_to_gdrive:
        checkpoint_utils.upload_checkpoints_to_gdrive(cfg, epoch_itr.epoch)
...
def validate(
    ...
    for subset in subsets:
        ...
        columns = ["Image", "Question", "Raw output", "Predicted answer", "Predicted explanation", "Target answer",
                   "Target explanation"]
        ...
        with metrics.aggregate(new_root=True) as agg:
            for i, sample in enumerate(progress):
                if cfg.dataset.max_valid_steps is not None and i > cfg.dataset.max_valid_steps:
                    break
                log_output = trainer.valid_step(sample, table=table)
                if log_output is not None:  # not OOM, overflow, ...
                    # log mid-epoch stats
                    if i % cfg.common.log_interval == 0:
                        ...
                        progress.log(stats, tag=subset, step=i)
        ...
        wandb.log({"Validation examples": table}, step=stats["num_updates"])
