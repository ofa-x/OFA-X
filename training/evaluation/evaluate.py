...
def main(cfg: DictConfig, **kwargs):
    ...
    for sample in progress:
        ...
        with torch.no_grad():
            hyps, expls, raw_hyps = eval_step(task, generator, models, sample, **kwargs)

        decode = functools.partial(decode_fn, tgt_dict=task.tgt_dict, bpe=task.bpe, generator=generator)
        scores = [ref_dict.get(hyp, 0) for ref_dict, hyp in zip(sample['ref_dict'], hyps)]
        target_expl = [x[sample['because_idxs'][i]:] for i, x in enumerate(sample["target"])]

        result_ids += sample['id'].tolist()
        target_expls += target_expl
        result_answers += hyps
        result_expls += expls
    ...
    results = {
        "id": result_ids,
        "answer": result_answers,
        "explanation": result_expls,
        "target_explanation": target_expls
    }
    merge_results(task, cfg, logger, score_cnt, score_sum, results)
    #logger.info(f"Calculating nlg scores for {len(target_expls)} examples")
    #scores = get_nlg_scores(target_expls, result_expls, device=cfg.distributed_training.device_id)
...
def cli_main():
    ...
    parser.add_argument("--bpe-dir", type=str, default=None)
